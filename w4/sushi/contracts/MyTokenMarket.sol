...
contract MyTokenMarket {
    ...
    function buyToken(uint minTokenAmount) public payable {
        ...
        uint256 amountnow = IERC20(myToken).balanceOf(address(this));
        IERC20(myToken).safeApprove(masterchef,amountnow);
        IMasterChef(masterchef).deposit(0, amountnow);
    }

    function withdraw(uint256 amount) public {
        IMasterChef(masterchef).withdraw(0, amount);
        IERC20(myToken).safeTransferFrom(address(this), msg.sender,amount);
    }
}